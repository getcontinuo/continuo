...
- **v0.0.4** (this release) -- L2 UltraRAG async integration. New `core/l2.py` module with `L2Config` (YAML + env-var overrides), `L2Client` Protocol, `FastMCPL2Client` production client backed by `fastmcp`, `query_l2()` entry point with timeout + graceful degradation. L2 is **disabled by default** so install works without UltraRAG; opt in via `core/l2_config.yaml` or `CONTINUO_L2_ENABLED=true`. Optional extra: `pip install 'continuo-memory[ultrarag]'` pulls fastmcp. 36 new tests (116 total) covering config loading, env-var precedence, client mocking, timeout, disabled-path, missing-fastmcp, orchestrator integration.
